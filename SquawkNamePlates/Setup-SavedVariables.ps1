...
if (-not (Test-Path $accounts)) {
    ...
    Write-Host "Run this from inside Interface\AddOns\SquawkNamePlates in your game folder."
    exit 1
}
...
Get-ChildItem $accounts -Directory |
    ...
    ForEach-Object {
        ...
        # do not exist until the first logout.  BigPvP.lua is the old name of
        # this addon, read once so settings carry over for anyone upgrading.
        foreach ($name in @("SquawkNamePlates.lua", "BigPvP.lua")) {
